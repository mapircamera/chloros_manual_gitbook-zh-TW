...
若将校准后的图像以Chloros（典型JPG格式）或TIFF输出，则反射率百分比通过像素值除以图像格式位深度计算得出：JPG格式除以255，TIFF格式除以65,535。 您也可选择Chloros的PERCENT格式输出，此时每个像素值将在0.0至1.0百分比范围内（即0%至100%反射率）。需注意部分图像应用程序无法处理百分比（浮点）图像，且此类图像存储占用较大空间。
