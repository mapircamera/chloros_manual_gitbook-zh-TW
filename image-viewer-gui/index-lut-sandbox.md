...
| **索引背景**       | 微调强调，保持数据上下文  | 范围着色，超出灰度 |
| **原始背景**     | 报告演示，直观分析         | 范围着色，超出照片 |

### 创建自定义LUT颜色

为实现可视化效果的完全掌控，您可通过编辑单个颜色停驻点创建**自定义颜色渐变**。
...
* **反转渐变**：颠倒颜色顺序以逆转含义（例如绿色=低值，红色=高值）
* **品牌色**：匹配组织报告的色彩方案
* **色盲友好**：采用橙蓝或紫黄配色组合
...
3. **应用 LUT 梯度** - 从 Red-Yellow-Green 预设开始
4. **检查像素值** - 移动光标观察数值范围
5. **调整最小/最大值** - 缩小范围聚焦植被（例如0.2至0.9）
6. **选择裁剪模式** - 尝试&quot;原始背景&quot;模式获取上下文
7. **精细调整色彩** - 根据特定强调需求定制渐变
...
* **观测值**： 土壤值 = 0.05-0.25，受胁值 = 0.25-0.50，健康值 = 0.50-0.85
* **目标**：仅可视化植物健康状况（排除土壤）
