...
* **较少分类**（3-5类）：形成鲜明分区，简化分类，便于辨识类别
* **中等类别数** (6-10)：平衡方案，适用于多数场景
* **较多类别数** (15-20)：平滑过渡，精细差异，呈现照片级效果

**适用场景：**

* **少量类别 (3-5)**：演示文稿、分类地图、简易报告
...
{%提示 style=&quot;info&quot; %}
**自动缩放**：首次应用LUT时，Chloros会自动将最小/最大值设为图像实际数据范围。您可随后缩小该范围以聚焦特定关注值域。
{% endhint %}
...
| **指数背景**       | 细微强调，保持数据关联性   | 范围着色，范围外灰色   |
| **原始背景**    | 报告演示，直观分析需求 | 范围着色，范围外照片 |
...
1. **选择指数层**（例如：RAW（反射率））
2. **应用指数** - 选择相机滤镜和指数公式，将彩色圆圈拖拽至指数公式的相应位置
...
* **观察结果**：土壤值 = 0.05-0.25，胁迫值 = 0.25-0.50，健康值 = 0.50-0.85
* **目标**：仅可视化植物健康状态 （排除土壤）
