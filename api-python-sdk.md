...
| `auto_start_backend`      | bool | `True`                    | 需要时自动启动后端 |
...
NDVI, NDRE, GNDVI, OSAVI, CIG, EVI, SAVI, MSAVI, MTVI2, 及其他。
...
**答：**仅初始授权激活时需要。通过Chloros、Chloros（浏览器）或Chloros CLI登录后，授权将本地缓存并支持离线使用30天。
...
此处列出的所有示例均经过测试且可投入生产环境。 请根据您的使用场景复制并调整代码。
